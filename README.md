...
### tester.py 만드는 법
runner.py를 돌리면 .pt 파일이 생성이 됨. 

이렇게 생성된 .pt 파일의 주소를 복사해서 tester.py에 붙여 넣고 runner 돌리는 것처럼 tester를 돌리게 되면 학습이 아니라 테스트용 지형 및 설정된 조이스틱 명령을 따라서 돌아감. 

기본적으로 B 키를 누르게 되면 env가 reset되면서 원점으로 돌아옴.
...
이후 ctrl + shift + p 하여 열린 창에 dev containers: Attach to running container를 입력하고 선택한다. (이걸 위해서는 vsc extension인 dev containers를 다운받아야 한다.)

그 후 원하는 컨테이너를 선택해서 접속하면 된다. -> 이러면 컨테이너 안으로 들어온 것.
